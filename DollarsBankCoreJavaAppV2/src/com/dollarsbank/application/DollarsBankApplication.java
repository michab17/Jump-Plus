...
import com.dollarsbank.controller.Menu;
...
//		name = input.nextLine();
//
//		List<String> usernameList = accountList.stream().map(Account::getUsername).toList();
//
...
//			}
//		}
//		Account newAccount = new Account(name, username, pin, availableFunds);
//
//		newAccount.transactions.add(new Transaction("Initial Deposit", "An initial deposit of $" + availableFunds + " was made when you created your account", Timestamp));
//
//		menu(input);
//	}
//
...
//		pin = input.nextLine();
//
//		Optional<Account> account = accountList.stream().filter(user -> user.getUsername().equals(username) && user.getPin().equals(pin)).findFirst();
//
